...
const SNIPPETS: Record<string, { template: string; preamble?: string }> = {
  ...
  'math-parts': { template: 'a) ${Visa att} $${y = x e^(-x)}$ ${löser ekvationen.}' },
  'math-answer': { template: 'svar ${y(x) = 3/4 e^(2x) + e^(-x)}' },
  'math-number': { template: "math #${ode} ${y'' - y' - 2y = x}" },
  'math-show': { template: 'show diff(${x^2*sin(x)}, ${x})' },
  ode: {
    template: "${y} = ode ${y'' - y' - 2y = x}, ${y(0) = 2}, ${y'(0) = 0} for ${x} from ${0.0} to ${3.0}",
  },
  roots: { template: '${r} = roots(${1, -1, -2})' },
  matrix: { template: '${A} = [${1, 2; 3, 4}]' },
  ...
  'math-paste': { template: 'math ${σ} ${≤} ${f_yd}' },
...
const LABELS: Record<string, string> = {
  ...
  'math-parts': 'part a), b), c)',
  'math-answer': 'boxed answer',
  'math-number': 'numbered equation',
  'math-show': 'symbolic derivative',
